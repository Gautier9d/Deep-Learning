...
def create_rationale_mask(input_ids: torch.Tensor, dog_whistle: str,
    ...
    for i, token in enumerate(all_tokens):
        ...
        if any(token.lower() == dwt.lower() or dwt.lower() in [token.lower()]
               or token.lower() in [dwt.lower()]
               for dwt in dog_whistle_tokens):
            rationale_mask[i] = 1
            continue
    ...
    return rationale_mask, all_tokens
...
def generate_rationale_mask(item, tokenizer, max_length):
    ...
    rationale_mask, all_tokens = create_rationale_mask(
        encoding["input_ids"][0], dog_whistle, tokenizer)

    if torch.sum(rationale_mask) == 0:
        print(text)
        print(dog_whistle)

    assert torch.sum(rationale_mask) > 0, "No rationale mask found"
